...
namespace ReminderApp.Application.Features.Commands.Comment.AddComment
{
    public class AddCommentCommandHandler : IRequestHandler<AddCommentCommand, bool>
    {
        ...
        public AddCommentCommandHandler(IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, IJwtTokenService jwtTokenService)
        {
        ...
        public async Task<bool> Handle(AddCommentCommand request, CancellationToken cancellationToken)
        {
            ...
            if (email is not null)
            {
                ...
                ReminderApp.Domain.Entities.Comment comment = new() { Star = request.AddCommentDto.Star, UserComment = request.AddCommentDto.Comment, Id = Guid.NewGuid(), CreatedDate = DateTime.Now, isActive = true, UserId = user.Id };
                ...
                if (user is not null)
                {
                    ...
                    existComment.UserComment = request.AddCommentDto.Comment;
